...
```ts
// [Before] ใช้ window แทน globalThis
window.location.href = `${API_URL}/auth/google`;

// [Before] ใช้ Promise.reject แทน throw
return Promise.reject(refreshError);

// [Before] sort() ไม่มี comparator function (CRITICAL BUG)
departments.sort();

// [Before] div ที่กดได้ แต่ไม่มี role และ keyboard handler
<div className="modal-overlay" onClick={closeModal}>

// [Before] label ไม่ได้เชื่อมกับ input
<label>Start Date</label>
<input type="date" ... />

// [Before] ใช้ replace() แทน replaceAll()
value.replace(/\D/g, "")

// [Before] ไม่มี readonly บน injected dependency
constructor(private reflector: Reflector) {}

// [Before] CSS มี semicolon เกิน และสีที่ contrast ต่ำเกินไป
color: #f87171; /* ความเปรียบต่างไม่ผ่าน WCAG AA */

// [Before] Import node:crypto ซ้ำ
import { randomUUID } from 'node:crypto';
import { createHash } from 'node:crypto';

// [Before] มี onClick แต่ไม่มี role="button" และ onKeyDown (Accessibility issue)
<span className="mini-avatar" onClick={handleClick}>A</span>

// [Before] Ambiguous spacing หลักจาก HTML element ใน JSX
</svg>{" "}My Leave
```
...
| BUG | 11 | แก้ไขแล้วทั้งหมด |
| VULNERABILITY / CRITICAL | 2 | แก้ไขแล้วทั้งหมด |
| **รวม** | **82** | **ครบ 100%** |
